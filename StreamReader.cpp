      ...
      m_bConnected( false )
{
    ...
    connect( &m_stratuxStatus, SIGNAL( disconnected() ), this, SLOT( stratuxDisconnected() ) );
}
...
{
    // Open the streams
    m_stratuxSituation.open( QUrl( QString( "ws://localhost/situation" ) ) );
    m_stratuxStatus.open( QUrl( QString( "ws://localhost/status" ) ) );
    m_stratuxTraffic.open( QUrl( QString( "ws://localhost/traffic" ) ) );
...
void StreamReader::disconnectStreams()
{
    ...
    emit newStatus( false, false, false );
}
...
void StreamReader::statusUpdate( const QString &qsMessage )
{
    ...
    emit newStatus( m_bStratuxStatus, m_bAHRSStatus, m_bGPSStatus );
}
...
void StreamReader::trafficUpdate( const QString &qsMessage )
{
    ...
    foreach( qsField, qslFields )
    {
        ...
        if( qsTag == "Icao_addr" )
            traffic.iICAO = iVal;   // Note this is not part of the struct
    ...
    if( (traffic.iICAO > 0) && traffic.bPosValid )
        emit newTraffic( traffic.iICAO, traffic );
}
...
{
    qDebug() << "Stratux GPS,AHRS,Traffic local websockets connected";
    m_bConnected = true;
}
...
{
    qDebug() << "Stratux websockets disconnected";
    emit newStatus( false, false, false );
    m_bConnected = false;
}
